...
export default function CalendarPicker({ selected, onChange }: Props) {
  ...
  if (dragStart && dragEnd) {
    getDatesBetween(dragStart, dragEnd).forEach(d => dragPreviewKeys.add(dateKey(d)))
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, textAlign: 'center' }}>
        {DAYS.map(d => (
          <div key={d} style={{ fontSize: 12, color: '#888', paddingBottom: 4 }}>{d}</div>
        ))}
        ...
        {days.map(date => {
          ...
          let bg = 'transparent'
          let color = isPast ? '#ccc' : 'inherit'
          if (isSelected) { bg = '#4f46e5'; color = 'white' }
          if (isPreview && !isSelected) { bg = '#c7d2fe'; color = '#1e1b4b' }
